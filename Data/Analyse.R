...
dfData$Slutdato <- as.Date(dfData$Slutdato, format = "%d-%m-%Y")
...
ggplot(dfSample, aes(x = Dato, y = CumRessourceomkostninger)) +
  geom_line() +
  labs(title = paste0("Kumulative ressourceomkostninger for ", dfSample$Sagsnr.,' - ',dfSample$Beskrivelse),
       x = "Dato",
...
ggplot(dfSample, aes(x = Dato, y = CumVareomkostninger)) +
  geom_line() +
  labs(title = paste0("Kumulative vareomkostninger for ", dfSample$Sagsnr.,' - ',dfSample$Beskrivelse),
       x = "Dato",
...
ggplot(dfSample, aes(x = Dato)) +
  ...
  labs(title = paste0("Omkostninger"),
...
dEnd <- as.numeric(tail(dfSample$Slutdato,1))
...
SSE <- function(dSkew, dPeak) {
  ...
  SSE <- sum((dfSample$TheoreticalScurve - dfSample$CumOmkostninger)^2)
  return(SSE)
}

# For each value of dSkew, calculate the sum of squared errors for different values of dPeak
dSkew <- seq(-1, 2, 0.1) # define the range of dSkew
dPeak <- seq(-1, 2, 0.1) # define the range of dPeak
...
# Ressources 
ggplot(dfSample, aes(x = Dato)) +
  geom_line(aes(y = TheoreticalScurveRes, color = "Teoretisk s-kurve")) +
  geom_line(aes(y = CumRessourceomkostninger, color = "Omkostninger")) +
...
# Total costs
ggplot(dfSample, aes(x = Dato)) +
  geom_line(aes(y = TheoreticalScurve, color = "Teoretisk s-kurve")) +
  geom_line(aes(y = CumOmkostninger, color = "Omkostninger")) +
